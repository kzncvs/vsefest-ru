...
## Архитектура: два уровня картинок, байты — вне git
...
|---|---|---|---|
| **Превью** | ресайз до 2000px, JPEG q82 | S3 (Yandex Object Storage) | ~0.4–0.9 МБ |
| **Оригинал** | как прислал фотограф | S3, та же папка `orig/` | МБ-ы |
...
- `preview` / `original` — ключи **относительно** `baseUrl`. Сменить бакет =
  поменять один `baseUrl`.
